...
更新 `agents/`、`rules/`、`commands/` 时，优先在用户偏好语言目录编写审核版，用户确认后再翻译到对应英文正式版。

| 类型 | 中文审核版目录 | 英文正式版目录 |
|------|----------------|----------------|
| Agent 提示词 | `docs/products/wopal-space-ontology/LANG/zh-CN/agents/` | `.wopal/agents/` |
| Rules | `docs/products/wopal-space-ontology/LANG/zh-CN/rules/` | `.wopal/rules/` |
| Commands | `docs/products/wopal-space-ontology/LANG/zh-CN/commands/` | `.wopal/commands/` |

约束：

1. 用户需要审核语义时，先改中文审核版，不直接改英文正式版。
2. 中文版确认后，再同步翻译到英文正式版。
3. 英文正式版是运行时加载源；中文审核版是语义确认源。
4. 若只修复英文正式版的格式、路径、frontmatter 或运行时加载问题，可直接改英文版，但不得改变未确认语义。
