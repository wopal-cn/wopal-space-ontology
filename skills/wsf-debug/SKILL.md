...
**Orchestrator role:** Gather symptoms, spawn wsf-debugger agent, handle checkpoints, spawn continuations.
...
```bash
debugger_model=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" resolve-model wsf-debugger --raw)
```
...
- Offer options:
  ...
  - "Plan fix" — suggest `/wsf-plan-phase --gaps`
  - "Manual fix" — done

**If `## DEBUG COMPLETE` (find_and_fix mode):**
- Display root cause and fix summary
- Offer options:
  - "Plan fix" — suggest `/wsf-plan-phase --gaps` if further work needed
  - "Done" — mark resolved
...
- [ ] wsf-debugger spawned with context
