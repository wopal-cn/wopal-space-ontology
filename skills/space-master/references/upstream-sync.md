...
│  └── 本地分支：main / type/* / space/<user>/<name> / contribute/*    │
└─────────────────────────────────────────────────────────────────────┘
                               │ worktree
                               ▼
┌─ 运行时 worktree ───────────────────────────────────────────────────┐
...
三层能力模型：`main`（通用层）→ `type/<name>`（类型层）→ `space/<user>/<name>`（空间层）。
...
| `space/<user>/<name>` | 空间实例分支，运行时 worktree 挂载 | 永久 |
