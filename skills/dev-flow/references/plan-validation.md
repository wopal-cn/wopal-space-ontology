...
|------|------|
| 文件命名 | 需符合 plan 命名规范 |
| 占位符 | 不得残留 TODO / FIXME / `path/to/` / `REQ-xxx` / TBD / TBC / `待补` 等 |
| **模板引导注释** | 不得残留 `<!-- ⚠️ ... -->` / `<!-- 自动注入 ... -->` / `<!-- WorktreeContext -->` 等模板 scaffolding |
...
**错误信息格式**：所有 issue 包含 `[Line N] problem + Fix: <hint> + Why: <reason>`，运行 `flow.sh plan check <name>` 一次性看完整错误。
