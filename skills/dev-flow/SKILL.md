...
`complete` 会输出规范路径 git status 和验证选项，agent 需将其传达给用户。

**验证选项**：
- **工作树验证**：用户直接在工作树目录验证功能，验证通过后 agent 执行 merge
- **分支切换验证**：执行 `flow.sh verify-switch <issue>` — 移除工作树 → 在规范路径 checkout 特性分支 → 更新 Plan 元数据并 commit → 用户在规范路径验证

**standard 项目**提供两种选项；**ontology-worktree 项目**仅提供分支切换（ellamaka 从 `.wopal/` 加载运行时能力，工作树内验证无意义）。

统一流程（分支切换路径）：
1. `flow.sh verify-switch <issue>` — 移除工作树 → checkout 特性分支到规范路径 → 更新 Plan Worktree 元数据（path → "(removed)"，新增 Verification Dir）→ commit Plan 变更
2. 用户验证功能
...
统一流程（工作树验证路径）：
1. 用户在工作树目录验证功能
2. 用户确认后，agent 在规范路径 merge feature → 集成分支
3. agent 在集成分支执行 `flow.sh verify <issue> --confirm`
4. agent 执行 `flow.sh archive <issue>`（archive 自动清理工作树）
