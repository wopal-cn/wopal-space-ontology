...
1. [ ] `rg -c 'fork' .wopal/skills/space-master/references/upstream-sync.md` 返回 0，或所有匹配项均在"fork-mode migration"上下文中（无 fork 作为默认模式的表述）
2. [ ] `rg -c 'space/<user>/<name>' .wopal/skills/space-master/references/upstream-sync.md` ≥ 1（新分支约定已文档化）
3. [ ] `rg -c 'wopal ontology' .wopal/skills/space-master/SKILL.md` ≥ 3（CLI 命令已引用）
4. [ ] `rg -c 'type/<name>' .wopal/skills/space-master/references/capability-layers.md` ≥ 1（type 分支已文档化）
5. [ ] `rg -c 'space save' .wopal/skills/space-master/SKILL.md` = 0（旧命令未被引用）
