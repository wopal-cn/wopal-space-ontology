...
- 正文只写项目特定技术实施规则。候选内容必须先分类为：当前事实、长期有效的目标结构约束、临时计划 / TODO。当前事实可写入架构；长期有效的目标结构约束可写入规则；临时计划 / TODO 不得写入 `AGENTS.md`。
...
- 保留基本开发 / 测试命令和适用验证要求；测试节必须包含 TDD 要求，并写清：哪些逻辑必须自动测试，哪些宿主 API、外部系统或真实运行环境边界只要求手工验证。
- 使用直接、可执行的祈使句；明确本范围负责什么、不应改什么。
- 描述用正向事实说明“是什么 / 负责什么”；少写防御性解释和低信息否定句。
- 用户偏好语言版本遵循 AGENTS 模板标题；模板规定的英文章节标题不得翻译。
- 架构说明只保留一条执行链和必要目录表；目录表只列当前真实存在的路径。长期有效的目标结构约束写入规则，不写成“当前目录事实”。禁止 README 式介绍、低信息适用范围句、PRD 愿景 / 用户叙事 / roadmap、业务规则复述、大段 DESIGN 原文、架构大图、目录百科、API / 命令 catalog、临时计划或命令记录链接（除非用户明确要求）。
...
11. 每条结构相关内容属于“当前事实 / 长期有效约束 / 临时计划”的归类说明

更新既有 `AGENTS.md` 时，规则与规范初次创建后不可直接修改；任何增删改都只能先作为建议进入方案，获得用户明确确认后才能执行。

若属于既有 `AGENTS.md` 的**小范围更新**（不改 frontmatter、不改目标路径、不改文档范围，只微调 1–2 个 section），方案可压缩为：目标文件、涉及章节、保留规则、拟新增 / 删除 / 合并规则、`User-Supplied Rules` 不变确认，以及结构相关内容的三分类说明。
...
- [ ] 架构目录表只列当前真实存在的路径；长期目标结构约束没有伪装成当前事实
...
- [ ] 测试节不仅有 TDD 表述，还写明了必须自动测试的逻辑类别与仅手工验证的边界
