...
There is no `/wsf-transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/wsf-transition`.

**Valid user commands for phase progression:**
- `/wsf-discuss-phase {N}` — discuss a phase before planning
- `/wsf-plan-phase {N}` — plan a phase
- `/wsf-execute-phase {N}` — execute a phase
- `/wsf-progress` — see roadmap progress
...
These will carry forward as debt. Review: `/wsf-audit-uat`
```
...
**Delegate ROADMAP.md and STATE.md updates to wsf-tools:**

```bash
TRANSITION=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `wsf-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" progress bar --raw)
```
...
**Use the transition result from `wsf-tools phase complete`:**
...
**Skip this check if NOT in workstream mode** (i.e., `WSF_WORKSTREAM` is not set / flat mode).
In flat mode, go directly to **Route B**.

```bash
# Only check if we're in workstream mode
if [ -n "$WSF_WORKSTREAM" ]; then
  WS_LIST=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" workstream list --raw)
fi
```
...
Filter out the current workstream (`$WSF_WORKSTREAM`) and any workstreams with
...
Exit skill and invoke SlashCommand("/wsf-plan-phase [X+1] --auto ${WSF_WS}")
...
Exit skill and invoke SlashCommand("/wsf-discuss-phase [X+1] --auto ${WSF_WS}")
...
`/wsf-discuss-phase [X+1] ${WSF_WS}` — gather context and clarify approach

---

**Also available:**
- `/wsf-plan-phase [X+1] ${WSF_WS}` — skip discussion, plan directly
- `/wsf-research-phase [X+1] ${WSF_WS}` — investigate unknowns
...
`/wsf-plan-phase [X+1] ${WSF_WS}`

---

**Also available:**
- `/wsf-discuss-phase [X+1] ${WSF_WS}` — revisit context
- `/wsf-research-phase [X+1] ${WSF_WS}` — investigate unknowns
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" config-set workflow._auto_chain_active false
```
...
`/wsf-workstreams complete {current_ws_name} ${WSF_WS}`

See overall milestone progress:

`/wsf-workstreams progress ${WSF_WS}`
...
Do NOT suggest `/wsf-complete-milestone` or `/wsf-new-milestone`.
...
Exit skill and invoke SlashCommand("/wsf-complete-milestone {version} ${WSF_WS}")
...
`/wsf-complete-milestone {version} ${WSF_WS}`
