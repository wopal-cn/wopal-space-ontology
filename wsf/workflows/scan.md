<purpose>
Lightweight codebase assessment. Spawns a single wsf-codebase-mapper agent for one focus area,
...
```bash
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" init map-codebase 2>/dev/null || echo "{}")
...
Spawn a single `wsf-codebase-mapper` agent with the selected focus area:
...
Use `/wsf-map-codebase` for a comprehensive 4-area parallel scan.
```
