...
<required_reading>
@/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/domain-probes.md
@/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/gate-prompts.md
@/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/universal-anti-patterns.md
</required_reading>

<downstream_awareness>
**CONTEXT.md feeds into:**

1. **wsf-phase-researcher** — Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **wsf-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
Enable text mode:
- Per-session: pass `--text` flag to any command (e.g., `/wsf-discuss-phase --text`)
- Per-project: `wsf-tools config-set workflow.text_mode true`
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/wsf-plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.

<step name="initialize" priority="first">
Phase number from argument (required).

```bash
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" init phase-op ${ARGUMENTS})
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ADVISOR=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" agent-skills wsf-advisor 2>/dev/null)
```
...
Use /wsf-progress ${WSF_WS} to see available phases.
```
Exit workflow.
...
- Read and execute @/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/workflows/discuss-phase-power.md end-to-end
...
- options:
  - "Continue and replan after" — Capture context, then run /wsf-plan-phase {X} ${WSF_WS} to replan
...
```bash
TODO_MATCHES=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" todo match-phase "${PHASE_NUMBER}")
```
...
1. Check for USER-PROFILE.md:
   ```bash
   PROFILE_PATH="/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/USER-PROFILE.md"
   ```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" resolve-model wsf-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @/Users/sam/coding/wopal/wopal-workspace/.wopal/agents/wsf-advisor-researcher.md for your role and instructions.
...
```bash
MAX_PASSES=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
This is a structured checkpoint, not the final CONTEXT.md — the `write_context` step still produces the canonical output. But if the session dies, the next `/wsf-discuss-phase` invocation can detect this checkpoint and offer to resume from it instead of starting from scratch.
...
`/wsf-plan-phase ${PHASE} ${WSF_WS}`

---

**Also available:**
- `/wsf-discuss-phase ${PHASE} --chain ${WSF_WS}` — re-run with auto plan+execute after
- `/wsf-plan-phase ${PHASE} --skip-research ${WSF_WS}` — plan without research
- `/wsf-ui-phase ${PHASE} ${WSF_WS}` — generate UI design contract before planning (if phase has frontend work)
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" state record-session \
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
```
Skill(skill="wsf-plan-phase", args="${PHASE} --auto ${WSF_WS}")
```
...
- **PHASE COMPLETE** → Full chain succeeded. Display:
  ...
  Next: /wsf-discuss-phase ${NEXT_PHASE} ${WAS_CHAIN ? "--chain" : "--auto"} ${WSF_WS}
  ```
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ```
  Auto-advance partial: Planning complete, execution did not finish.
  Continue: /wsf-execute-phase ${PHASE} ${WSF_WS}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /wsf-plan-phase ${PHASE} ${WSF_WS}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /wsf-plan-phase ${PHASE} --gaps ${WSF_WS}
  ```
...
**Full step-by-step instructions:** @/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/workflows/discuss-phase-power.md
