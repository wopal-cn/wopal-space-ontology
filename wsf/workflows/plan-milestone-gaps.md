<purpose>
Create all phases necessary to close gaps identified by `/wsf-audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/wsf-add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/wsf-audit-milestone` first.
```
...
HIGHEST=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" phases list --pick directories[-1])
```
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/wsf-execute-phase {N}` — if plans already exist
...
`/wsf-audit-milestone` — re-audit to verify gaps closed
`/wsf-complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/wsf-plan-phase` next
</success_criteria>
