<purpose>
Safe git revert workflow. Rolls back WSF phase or plan commits using the phase manifest with dependency checks and a confirmation gate. Uses git revert --no-commit (NEVER git reset) to preserve history.
</purpose>
...
```bash
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" init undo "$ARGUMENTS")
...
```
Usage: /wsf-undo --last N | --phase NN | --plan NN-MM

Modes:
  --last N      Show last N WSF commits for interactive selection
...
Filter for WSF conventional commits matching `type(scope): message` pattern (e.g., `feat(04-01):`, `docs(03):`, `fix(02-03):`).
...
```
Working tree has uncommitted changes. Commit or stash them before running /wsf-undo.
```
...
**Also available:**
- `/wsf-execute-phase ${PHASE}` — re-execute if needed
- `/wsf-undo --last 1` — undo the revert itself if something went wrong
