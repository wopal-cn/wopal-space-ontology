...
```bash
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" init map-codebase $ARGUMENTS)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" agent-skills wsf-codebase-mapper 2>/dev/null)
```
...
See also: @/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/subagent-tool-adapter.md
</step>
...
**CRITICAL:** Use the dedicated `wsf-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.

**Agent 1: Tech Focus**

```
Task(
  subagent_type="wsf-codebase-mapper",
  model="{mapper_model}",
...
Use the same document templates as the `wsf-codebase-mapper` agent. Include actual file paths formatted with backticks.
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
...
**Also available:**
- Re-run mapping: `/wsf-map-codebase`
...
- If Task tool available: 4 parallel wsf-codebase-mapper agents spawned with run_in_background=true
- If Task tool NOT available: 4 sequential mapping passes performed inline (never using browser_subagent)
- All 7 codebase documents exist
- No empty documents (each should have >20 lines)
- Clear completion summary with line counts
- User offered clear next steps in WSF style
</success_criteria>
