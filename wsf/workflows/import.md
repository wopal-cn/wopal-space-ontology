...
- **--from**: Import external plan → conflict detection → write PLAN.md → validate via wsf-plan-checker
...
- If `--prd` is present: display message that `--prd` is not yet implemented and exit:
  ```
  WSF > --prd mode is planned for a future release. Use --from to import plan files.
  ```
- If neither flag is found: display usage and exit:

```
Usage: /wsf-import --from <path>

  --from <path>   Import an external plan file into WSF format
```
...
2. Read `.planning/PROJECT.md` — extract project constraints, tech stack, scope boundaries.
   **If PROJECT.md does not exist:** skip constraint checks that rely on it and display:
   ```
   WSF > Note: No PROJECT.md found. Conflict checks against project constraints will be skipped.
   ```
...
- **WSF PLAN.md format**: Has YAML frontmatter with `phase:`, `plan:`, `type:` fields
...
Display:
```
WSF > BLOCKED: {N} blockers must be resolved before import can proceed.
```
...
Convert the imported content to WSF PLAN.md format.
...
If the imported plan references PBR plan naming (e.g., `PLAN-01.md`, `plan-01.md`), rename all references to WSF `{NN}-{MM}-PLAN.md` convention during conversion.

Apply WSF naming convention for the output filename:
...
Delegate validation to wsf-plan-checker:

```
Task({
  subagent_type: "wsf-plan-checker",
  prompt: "Validate: .planning/phases/{phase}/{plan}-PLAN.md — check frontmatter completeness, task structure, and WSF conventions. Report any issues."
})
```
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" commit "docs({phase}): import plan from {basename FILEPATH}" --files .planning/phases/{phase}/{plan}-PLAN.md .planning/ROADMAP.md
```
...
- Use `pbr:plan-checker` or `pbr:planner` — use `wsf-plan-checker` and `wsf-planner`
- Write `.planning/.active-skill` — this is a PBR pattern with no WSF equivalent
