...
```bash
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" init manager)
...
- `manager_flags.discuss` — appended to `/wsf-discuss-phase` args (e.g. `"--auto --analyze"`)
- `manager_flags.plan` — appended to plan agent init command
- `manager_flags.execute` — appended to execute agent init command

These are empty strings by default. Set via: `wsf-tools config-set manager.flags.discuss "--auto --analyze"`
...
All {phase_count} phases done. Ready for final steps:
  → /wsf-verify-work — run acceptance testing
  → /wsf-complete-milestone — archive and wrap up
```
...
Handle responses:
- "Verify work": `Skill(skill="wsf-verify-work")`  then loop to dashboard.
- "Complete milestone": `Skill(skill="wsf-complete-milestone")` then exit.
...
**Auto-refresh:** If background agents are running (`is_active` is true for any phase), set a 60-second auto-refresh cycle. After presenting the action menu, if no user input is received within 60 seconds, automatically refresh the dashboard. This interval is configurable via `manager_refresh_interval` in WSF config (default: 60 seconds, set to 0 to disable).
...
```
Skill(skill="wsf-discuss-phase", args="{PHASE_NUM} {manager_flags.discuss}")
```
...
- Display the error clearly, then offer to fix it:
  ...
  - "Run this phase inline instead": Dispatch the same action inline via the appropriate Skill — use `Skill(skill="wsf-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="wsf-execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
...
- Display the error, then offer options via AskUserQuestion:
  ...
  - "Run inline instead": Dispatch the action inline via the appropriate Skill — use `Skill(skill="wsf-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="wsf-execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
  - "Skip and continue": Loop to dashboard (phase stays in current state).
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume anytime: /wsf-manager
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Note:** Any background agents still running will continue to completion. Their results will be visible on next `/wsf-manager` or `/wsf-progress` invocation.
