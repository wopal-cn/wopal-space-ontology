...
```bash
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" init health "$ARGUMENTS")
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" validate health $REPAIR_FLAG
```
...
- [E001] config.json: JSON parse error at line 5
  Fix: Run /wsf-health --repair to reset to defaults

- [E002] PROJECT.md not found
  Fix: Run /wsf-new-project to create
```
...
```
---
N issues can be auto-repaired. Run: /wsf-health --repair
```
</step>
...
```
Would you like to run /wsf-health --repair to fix N issues automatically?
```
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" validate health
```
...
TASKS_DIR="/Users/sam/coding/wopal/wopal-workspace/.wopal/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /Users/sam/coding/wopal/wopal-workspace/.wopal/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /Users/sam/coding/wopal/wopal-workspace/.wopal/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
