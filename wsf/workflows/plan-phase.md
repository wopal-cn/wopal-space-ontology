<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates wsf-phase-researcher, wsf-planner, and wsf-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
@/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/revision-loop.md
...
```bash
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" init plan-phase $ARGUMENTS)
...
AGENT_SKILLS_PLANNER=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" agent-skills wsf-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" agent-skills wsf-checker 2>/dev/null)
...
**If `planning_exists` is false:** Error — run `/wsf-new-project` first.
...
Then re-run /wsf-plan-phase {N} --reviews
```
Exit workflow.
...
5. Commit:
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
DISCUSS_MODE=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call — AskUserQuestion
  ...
  ```
  Run this command first, then re-run /wsf-plan-phase {X} ${WSF_WS}:

  /wsf-discuss-phase {X} ${WSF_WS}
  ```
...
```bash
PHASE_DESC=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" roadmap get-phase "${PHASE}" --pick section)
```
...
**If found:**
1. Read template: `/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/templates/VALIDATION.md`
...
SECURITY_ASVS=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 WSF ► SECURITY THREAT MODEL REQUIRED (ASVS L{SECURITY_ASVS})
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```bash
UI_PHASE_CFG=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" roadmap get-phase "${PHASE}" 2>/dev/null)
...
```
Skill(skill="wsf-ui-phase", args="${PHASE} --auto ${WSF_WS}")
```
After `wsf-ui-phase` returns, re-read:
```bash
UI_SPEC_FILE=$(ls "${PHASE_DIR}"/*-UI-SPEC.md 2>/dev/null | head -1)
...
1. Generate UI-SPEC first — Run /wsf-ui-phase {N} then re-run /wsf-plan-phase {N}
...
- options:
  - "Generate UI-SPEC first" → Display: "Run `/wsf-ui-phase {N} ${WSF_WS}` then re-run `/wsf-plan-phase {N} ${WSF_WS}`". Exit workflow.
...
```bash
PHASE_SECTION=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" roadmap get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
```bash
_wsf_field() { node -e "const o=JSON.parse(process.argv[1]); const v=o[process.argv[2]]; process.stdout.write(v==null?'':String(v))" "$1" "$2"; }
STATE_PATH=$(_wsf_field "$INIT" state_path)
ROADMAP_PATH=$(_wsf_field "$INIT" roadmap_path)
REQUIREMENTS_PATH=$(_wsf_field "$INIT" requirements_path)
RESEARCH_PATH=$(_wsf_field "$INIT" research_path)
VERIFICATION_PATH=$(_wsf_field "$INIT" verification_path)
UAT_PATH=$(_wsf_field "$INIT" uat_path)
CONTEXT_PATH=$(_wsf_field "$INIT" context_path)
REVIEWS_PATH=$(_wsf_field "$INIT" reviews_path)
```
...
2. Disable Nyquist with the exact command:
   `node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-set workflow.nyquist_validation false`
3. Continue anyway (plans fail Dimension 8)

Proceed to Step 8 only if user selects 2 or 3.

## 8. Spawn wsf-planner Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 WSF ► PLANNING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
<downstream_consumer>
Output consumed by /wsf-execute-phase. Plans need:
...
**If "Split":** Use `/wsf-insert-phase` to create the sub-phases, then replan each.
...
## 10. Spawn wsf-plan-checker Agent
...
Task(
  ...
  subagent_type="wsf-plan-checker",
  model="{checker_model}",
...
**Stall detection:** If `issue_count >= prev_issue_count`:
  ...
  **If `stall_reentry_count >= 2`:**
    ...
    Suggest: "Consider resolving these issues manually or running `/wsf-debug` to investigate root causes."
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" state planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
if ([[ "$ARGUMENTS" =~ --auto ]] || [[ "$ARGUMENTS" =~ --chain ]]) && [[ "$AUTO_CHAIN" != "true" ]]; then
  node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-set workflow._auto_chain_active true
fi
```
...
```
Skill(skill="wsf-execute-phase", args="${PHASE} --auto --no-transition ${WSF_WS}")
```
...
- **PHASE COMPLETE** → Display final summary:
  ```
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   WSF ► PHASE ${PHASE} COMPLETE ✓
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  Auto-advance pipeline finished.

  Next: /wsf-discuss-phase ${NEXT_PHASE} --auto ${WSF_WS}
  ```
- **GAPS FOUND / VERIFICATION FAILED** → Display result, stop chain:
  ...
  /wsf-execute-phase ${PHASE} ${WSF_WS}
  ```
...
/wsf-execute-phase {X} ${WSF_WS}

───────────────────────────────────────────────────────────────

**Also available:**
- cat .planning/phases/{phase-dir}/*-PLAN.md — review plans
- /wsf-plan-phase {X} --research — re-research first
- /wsf-review --phase {X} --all — peer review plans with external AIs
- /wsf-plan-phase {X} --reviews — replan incorporating review feedback
...
5. **Retry:** Restart Claude Code and run `/wsf-plan-phase` again

If freezes persist, try `--skip-research` to reduce the agent chain from 3 to 2 agents:
```
/wsf-plan-phase N --skip-research
```
</windows_troubleshooting>
...
- [ ] wsf-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] wsf-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] wsf-plan-checker spawned with CONTEXT.md
