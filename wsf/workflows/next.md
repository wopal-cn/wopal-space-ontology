<purpose>
Detect current project state and automatically advance to the next logical WSF workflow step.
...
```bash
# Get state snapshot
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" state json 2>/dev/null || echo "{}"
```
...
```
No WSF project detected. Run `/wsf-new-project` to get started.
```
Exit.
</step>
...
Run `/wsf-health` to diagnose, or manually fix STATE.md.
Use `--force` to bypass this check.
```
Exit.
...
- If file exists and count >= 6: prompt "You've called /wsf-next {N} times consecutively. Continue? [y/N]"
- If user says no, exit
- Increment the counter
- The counter file is deleted by any non-`/wsf-next` command (convention — other workflows don't need to implement this, the note here is sufficient)
</step>
...
→ Next action: `/wsf-discuss-phase <first-phase>`
...
▶ **Next step:** `/wsf-[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/wsf-next` is zero-friction advancement.
</step>
