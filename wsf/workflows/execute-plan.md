...
@/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" init execute-phase "${PHASE}")
...
# config settings can be fetched via wsf-tools config-get if needed
```
...
2. Per segment:
   - Subagent route: spawn wsf-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" phases list --type summaries --raw
...
See `/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/tdd.md` for structure.
</tdd_plan_execution>
...
```bash
node /Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See /Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node "./.wopal/wsf/bin/wsf-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.wopal/wsf/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/templates/summary.md`.
...
Update STATE.md using wsf-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" state update-progress

# Record execution metrics
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" state record-metric \
...
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using wsf-tools:

```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" state record-session \
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/wsf-execute-phase {phase}` + `/wsf-verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/wsf-plan-phase {Z+1}` + `/wsf-verify-work {Z}` + `/wsf-discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/wsf-complete-milestone` + `/wsf-verify-work` + `/wsf-add-phase` |
