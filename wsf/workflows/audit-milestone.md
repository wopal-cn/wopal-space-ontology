...
AGENT_SKILLS_CHECKER=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" agent-skills wsf-integration-checker 2>/dev/null)
```
...
```bash
integration_checker_model=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" resolve-model wsf-integration-checker --raw)
```
...
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" phases list
```
...
PHASE_INFO=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" find-phase 01 --raw)
...
${AGENT_SKILLS_CHECKER}",
  subagent_type="wsf-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" config-get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/wsf-validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/wsf-validate-phase {N}` |

Phases needing validation: run `/wsf-validate-phase {N}` for each flagged phase.
...
- /wsf-complete-milestone {version} — proceed anyway (accept tech debt)
