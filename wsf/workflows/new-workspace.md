...
```bash
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" init new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /wsf-new-workspace --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /wsf-new-workspace --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd $TARGET_PATH
  /wsf-new-project    # Initialize WSF in the workspace
```
...
**Offer to initialize WSF (if not `--auto`):**

Use AskUserQuestion:
- header: "Initialize WSF"
- question: "Would you like to initialize a WSF project in the new workspace?"
- options:
  - "Yes — run /wsf-new-project" → tell user to `cd $TARGET_PATH` first, then run `/wsf-new-project`
