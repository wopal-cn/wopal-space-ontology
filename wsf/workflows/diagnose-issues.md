...
```bash
USE_WORKTREES=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
```bash
AGENT_SKILLS_DEBUGGER=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" agent-skills wsf-debugger 2>/dev/null)
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" commit "docs({phase_num}): add root causes from diagnosis" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
