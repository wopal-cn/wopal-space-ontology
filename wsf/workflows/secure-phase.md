...
- wsf-security-auditor — Verifies threat mitigation coverage
</available_agent_types>
...
AGENT_SKILLS_AUDITOR=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" agent-skills wsf-security-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" resolve-model wsf-security-auditor --raw)
SECURITY_CFG=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
```

If `SECURITY_CFG` is `false`: exit with "Security enforcement disabled. Enable via /wsf-settings."

Display banner: `WSF > SECURE PHASE {N}: {name}`
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /wsf-execute-phase {N} first."
...
## 5. Spawn wsf-security-auditor

```
Task(
  prompt="Read /Users/sam/coding/wopal/wopal-workspace/.wopal/agents/wsf-security-auditor.md for instructions.\n\n" +
  ...
  subagent_type="wsf-security-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/templates/SECURITY.md`
...
```
WSF > PHASE {N} SECURITY BLOCKED
{K} threats open — phase advancement blocked until threats_open: 0
▶ Fix mitigations then re-run: /wsf-secure-phase {N}
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" commit "docs(phase-${PHASE}): add/update security threat verification"
```
...
```
WSF > PHASE {N} THREAT-SECURE
threats_open: 0 — all threats have dispositions.
▶ /wsf-validate-phase {N}    validate test coverage
▶ /wsf-verify-work {N}       run UAT
```
