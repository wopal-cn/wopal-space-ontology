...
AGENT_SKILLS_AUDITOR=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" agent-skills wsf-nyquist-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" resolve-model wsf-nyquist-auditor --raw)
NYQUIST_CFG=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" config-get workflow.nyquist_validation --raw)
```

If `NYQUIST_CFG` is `false`: exit with "Nyquist validation is disabled. Enable via /wsf-settings."

Display banner: `WSF > VALIDATE PHASE {N}: {name}`
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /wsf-execute-phase {N} ${WSF_WS} first."
...
## 5. Spawn wsf-nyquist-auditor

```
Task(
  prompt="Read /Users/sam/coding/wopal/wopal-workspace/.wopal/agents/wsf-nyquist-auditor.md for instructions.\n\n" +
    "<files_to_read>{PLAN, SUMMARY, impl files, VALIDATION.md}</files_to_read>" +
    "<gaps>{gap list}</gaps>" +
    "<test_infrastructure>{framework, config, commands}</test_infrastructure>" +
    "<constraints>Never modify impl files. Max 3 debug iterations. Escalate impl bugs.</constraints>" +
    "${AGENT_SKILLS_AUDITOR}",
  subagent_type="wsf-nyquist-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/templates/VALIDATION.md`
...
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
...
**Compliant:**
```
WSF > PHASE {N} IS NYQUIST-COMPLIANT
All requirements have automated verification.
▶ Next: /wsf-audit-milestone ${WSF_WS}
```

**Partial:**
```
WSF > PHASE {N} VALIDATED (PARTIAL)
{M} automated, {K} manual-only.
▶ Retry: /wsf-validate-phase {N} ${WSF_WS}
```
