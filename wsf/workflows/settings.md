<purpose>
Interactive configuration of WSF workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.wsf/defaults.json) for future projects.
</purpose>
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" config-ensure-section
...
- `workflow.ui_safety_gate` — prompt to run /wsf-ui-phase before planning frontend phases (default: true if absent)
...
AskUserQuestion([
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /wsf-ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /wsf-ui-phase first when frontend indicators detected." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Per Phase", description: "Create branch for each phase (wsf/phase-{N}-{name})" },
      { label: "Per Milestone", description: "Create branch for entire milestone (wsf/{version}-{name})" }
    ]
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Skip discuss in /wsf-autonomous — chain directly to plan. Best for backend/pipeline work where phase descriptions are the spec." }
    ]
  },
  {
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.wsf/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.wsf/defaults.json`:

```bash
mkdir -p ~/.wsf
```

Write `~/.wsf/defaults.json` with:
```json
{
  "mode": <current>,
  "granularity": <current>,
...
These settings apply to future /wsf-plan-phase and /wsf-execute-phase runs.

Quick commands:
- /wsf-set-profile <profile> — switch model profile
- /wsf-plan-phase --research — force research
- /wsf-plan-phase --skip-research — skip research
- /wsf-plan-phase --skip-verify — skip plan check
```
</step>
...
- [ ] User offered to save as global defaults (~/.wsf/defaults.json)
