<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /wsf-plan-phase --gaps.
...
```bash
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" init verify-work ${ARGUMENTS})
...
Provide a phase number to start testing (e.g., /wsf-verify-work 4)
```
...
```
UI_PHASE_FLAG=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
```
⚠ Security enforcement enabled — /wsf-secure-phase {phase} has not run.
Run before advancing to the next phase.

All tests passed. Ready to continue.

- `/wsf-secure-phase {phase}` — security review (required before advancing)
- `/wsf-plan-phase {next}` — Plan next phase
- `/wsf-execute-phase {next}` — Execute next phase
- `/wsf-ui-review {phase}` — visual quality audit (if frontend files were modified)
```
...
- `/wsf-plan-phase {next}` — Plan next phase
- `/wsf-execute-phase {next}` — Execute next phase
- `/wsf-secure-phase {phase}` — security review
- `/wsf-ui-review {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
- Follow @/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/workflows/diagnose-issues.md
...
Spawn wsf-planner in --gaps mode:
...
<downstream_consumer>
Output consumed by /wsf-execute-phase
...
Spawn wsf-planner with revision context:
...
3. Abandon (exit, user runs /wsf-plan-phase manually)
...
`/clear` then `/wsf-execute-phase {phase} --gaps-only`
...
- [ ] If issues: wsf-planner creates fix plans (gap_closure mode)
- [ ] If issues: wsf-plan-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/wsf-execute-phase --gaps-only` when complete
</success_criteria>
