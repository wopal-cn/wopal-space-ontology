<purpose>
Review source files changed during a phase for bugs, security issues, and code quality problems. Computes file scope (--files override > SUMMARY.md > git diff fallback), checks config gate, spawns wsf-code-reviewer agent, commits REVIEW.md, and presents results to user.
</purpose>
...
- wsf-code-reviewer: Reviews source files for bugs and quality issues
</available_agent_types>
...
```
Error: Phase ${PHASE_ARG} not found. Run /wsf-status to see available phases.
```
...
2. Config value: `node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" config-get workflow.code_review_depth 2>/dev/null`
3. Default: "standard"
...
if [ -n "$DEPTH_OVERRIDE" ]; then
  ...
  CONFIG_DEPTH=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get workflow.code_review_depth 2>/dev/null || echo "")
...
if [ ${#REVIEW_FILES[@]} -eq 0 ]; then
  ...
  if [ -n "$PHASE_COMMITS" ]; then
    ...
    echo "Use --files flag to specify files explicitly: /wsf-code-review ${PHASE_ARG} --files=file1,file2,..."
  fi
fi
```
...
Spawn the wsf-code-reviewer agent:

```
Task(subagent_type="wsf-code-reviewer", prompt="
<files_to_read>
${FILES_TO_READ}
</files_to_read>
...
No REVIEW.md created. You can retry with /wsf-code-review ${PHASE_ARG} or check agent logs.
```
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  if [ "$HAS_STATUS" = "valid" ]; then
    ...
    if [ "$COMMIT_DOCS" = "true" ]; then
      node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" commit \
  ...
  echo "No REVIEW.md to commit. Please retry with /wsf-code-review ${PHASE_ARG}"
fi
```
</step>
...
Next steps:
  /wsf-code-review-fix ${PHASE_NUMBER}  — Auto-fix issues
