...
Write to a temp file: `/tmp/wsf-review-prompt-{phase}.md`
</step>
...
**Gemini:**
```bash
gemini -p "$(cat /tmp/wsf-review-prompt-{phase}.md)" 2>/dev/null > /tmp/wsf-review-gemini-{phase}.md
```

**Claude (separate session):**
```bash
claude -p "$(cat /tmp/wsf-review-prompt-{phase}.md)" 2>/dev/null > /tmp/wsf-review-claude-{phase}.md
```

**Codex:**
```bash
codex exec --skip-git-repo-check "$(cat /tmp/wsf-review-prompt-{phase}.md)" 2>/dev/null > /tmp/wsf-review-codex-{phase}.md
```
...
```bash
coderabbit review --prompt-only 2>/dev/null > /tmp/wsf-review-coderabbit-{phase}.md
```

**OpenCode (via GitHub Copilot):**
```bash
cat /tmp/wsf-review-prompt-{phase}.md | opencode run - 2>/dev/null > /tmp/wsf-review-opencode-{phase}.md
if [ ! -s /tmp/wsf-review-opencode-{phase}.md ]; then
  echo "OpenCode review failed or returned empty output." > /tmp/wsf-review-opencode-{phase}.md
fi
```
...
Commit:
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
- [ ] User knows how to use feedback (/wsf-plan-phase --reviews)
</success_criteria>
