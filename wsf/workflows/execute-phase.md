...
See also:
- @/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/runtime-detection.md
- @/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/subagent-tool-adapter.md
</runtime_compatibility>

<required_reading>
Read STATE.md before any operation to load project context.

@/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/agent-contracts.md
@/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/context-budget.md
@/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/gates.md
</required_reading>

<available_agent_types>
These are the valid WSF subagent types registered in .claude/agents/ (or equivalent for your runtime).
Always use the exact name from this list — do not fall back to 'general-purpose' or other built-in types:

- wsf-executor — Executes plan tasks, commits, creates SUMMARY.md
- wsf-verifier — Verifies phase completion, checks quality gates
- wsf-planner — Creates detailed plans from phase scope
- wsf-phase-researcher — Researches technical approaches for a phase
- wsf-plan-checker — Reviews plan quality before execution
- wsf-debugger — Diagnoses and fixes issues
- wsf-codebase-mapper — Maps project structure and dependencies
- wsf-integration-checker — Checks cross-phase integration
- wsf-nyquist-auditor — Validates verification coverage
- wsf-ui-researcher — Researches UI/UX approaches
- wsf-ui-checker — Reviews UI implementation quality
- wsf-ui-auditor — Audits UI against design requirements
</available_agent_types>
...
```bash
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" init execute-phase ${ARGUMENTS})
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" agent-skills wsf-executor 2>/dev/null)
```
...
```bash
USE_WORKTREES=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
```bash
CONTEXT_WINDOW=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get context_window 2>/dev/null || echo "200000")
```
...
See also: @/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/runtime-detection.md

**REQUIRED — Sync chain flag with intent.** If user invoked manually (no `--auto`), clear the ephemeral chain flag from any previous interrupted `--auto` chain. This prevents stale `_auto_chain_active: true` from causing unwanted auto-advance. This does NOT touch `workflow.auto_advance` (the user's persistent settings preference). You MUST execute this bash block before any config reads:
```bash
# REQUIRED: prevents stale auto-chain from previous --auto runs
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/workflows/execute-plan.md` **inline**
...
- Maintains WSF's planning/tracking structure
- Best for: small phases, bug fixes, verification gaps, learning WSF
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" phase-plan-index "${PHASE_NUMBER}")
```
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       For wsf-tools commits: add --no-verify flag.
       For direct git commits: use git commit --no-verify -m "..."
       </parallel_execution>

       <execution_context>
       @/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/workflows/execute-plan.md
       @/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/templates/summary.md
       @/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/checkpoints.md
       @/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/references/tdd.md
       </execution_context>
   ...
   for WT in $WORKTREES; do
     ...
     if [ -n "$WT_BRANCH" ] && [ "$WT_BRANCH" != "HEAD" ]; then
       ...
       if ! git diff --quiet .planning/STATE.md .planning/ROADMAP.md 2>/dev/null || \
         ...
         COMMIT_DOCS=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get commit_docs 2>/dev/null || echo "true")
...
5.6. **Post-wave shared artifact update (worktree mode only):**
   ...
   for PLAN_ID in ${WAVE_PLAN_IDS}; do
     node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" roadmap update-plan-progress "${PHASE_NUMBER}" "${PLAN_ID}" completed
   done
...
7. **Handle failures:**
   ...
   **Known Claude Code bug (classifyHandoffIfNeeded):** If an agent reports "failed" with error containing `classifyHandoffIfNeeded is not defined`, this is a Claude Code runtime bug — not a WSF or agent issue. The error fires in the completion handler AFTER all tool calls finish. In this case: run the same spot-checks as step 5 (SUMMARY.md exists, git commits present, no Self-Check: FAILED). If spot-checks PASS → treat as **successful**. If spot-checks FAIL → treat as real failure below.
...
7b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
**Security gate check:**
```bash
SECURITY_CFG=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
...
```bash
POST_PLAN_INDEX=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" phase-plan-index "${PHASE_NUMBER}")
```
...
/wsf-execute-phase {phase} ${WSF_WS}                # Continue remaining waves
/wsf-execute-phase {phase} --wave {next} ${WSF_WS}  # Run the next wave explicitly
```
...
**Config gate:**
```bash
CODE_REVIEW_ENABLED=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get workflow.code_review 2>/dev/null || echo "true")
```
...
**Invoke review:**
```
Skill(skill="wsf-code-review", args="${PHASE_NUMBER}")
```
...
/wsf-code-review-fix ${PHASE_NUMBER}
```
...
```bash
PARENT_INFO=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
SCHEMA_DRIFT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" verify schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
...
Check for override:
```bash
SKIP_SCHEMA=$(echo "${WSF_SKIP_SCHEMA_CHECK:-false}")
```
...
Display:
```
⚠ Schema drift detected but WSF_SKIP_SCHEMA_CHECK=true — bypassing gate.
...
2. Skip schema check (WSF_SKIP_SCHEMA_CHECK=true) — bypass this gate
...
```bash
VERIFIER_SKILLS=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" agent-skills wsf-verifier 2>/dev/null)
```
...
| `gaps_found` | Present gap summary, offer `/wsf-plan-phase {phase} --gaps ${WSF_WS}` |
...
Commit the file:
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
Items saved to `{phase_num}-HUMAN-UAT.md` — they will appear in `/wsf-progress` and `/wsf-audit-uat`.

"approved" → continue | Report issues → gap closure
```

**If user says "approved":** Proceed to `update_roadmap`. The HUMAN-UAT.md file persists with `status: partial` and will surface in future progress checks until the user runs `/wsf-verify-work` on it.
...
`/wsf-plan-phase {X} --gaps ${WSF_WS}`

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` — full report
Also: `/wsf-verify-work {X} ${WSF_WS}` — manual testing first
```

Gap closure cycle: `/wsf-plan-phase {X} --gaps ${WSF_WS}` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/wsf-execute-phase {X} --gaps-only ${WSF_WS}` → verifier re-runs.
</step>
...
```bash
COMPLETION=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" phase complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/wsf-progress` and `/wsf-audit-uat`.
```

```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
entries from the completed phase to the global learnings store at `~/.wsf/knowledge/`.

**Check config gate:**
```bash
GL_ENABLED=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get features.global_learnings --raw 2>/dev/null || echo "false")
```
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" learnings copy 2>/dev/null || echo "⚠ Learnings copy failed — continuing"
```
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/wsf-plan-phase {X} --gaps`). No additional routing needed — skip auto-advance.
...
Read and follow `/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If none of `--auto`, `AUTO_CHAIN`, or `AUTO_CFG` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/wsf-transition` command. Never suggest it. The transition workflow is internal only.**

```
## ✓ Phase {X}: {Name} Complete

/wsf-progress ${WSF_WS} — see updated roadmap
/wsf-discuss-phase {next} ${WSF_WS} — discuss next phase before planning
/wsf-plan-phase {next} ${WSF_WS} — plan next phase
/wsf-execute-phase {next} ${WSF_WS} — execute next phase
```
...
<failure_handling>
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` → Claude Code bug, not WSF. Spot-check (SUMMARY exists, commits present) → if pass, treat as success
...
<resumption>
Re-run `/wsf-execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
