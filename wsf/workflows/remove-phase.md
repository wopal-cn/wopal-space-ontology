...
```bash
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" init remove-phase $ARGUMENTS)
...
To abandon current work, use /wsf-pause-work instead.
```
...
```bash
RESULT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), wsf-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" phase remove "${target}" --force)
```
...
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
...
Would you like to:
- `/wsf-progress` — see updated roadmap status
...
- Don't manually renumber — use `wsf-tools phase remove` which handles all renumbering
...
- [ ] `wsf-tools phase remove` executed successfully
