...
```bash
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" init complete-milestone $ARGUMENTS)
...
2. **Run audit first** — `/wsf-audit-milestone` to assess gap severity
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `wsf-tools milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
```bash
ARCHIVE=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
If "Skip": Phase directories remain in `.planning/phases/` as raw execution history. Use `/wsf-cleanup` later to archive retroactively.
...
**If doesn't exist:** Create from template at `/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/templates/retrospective.md`.
...
**Commit:**
```bash
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
Detect base branch:
```bash
BASE_BRANCH=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get git.base_branch 2>/dev/null || echo "")
...
- [ ] User knows next step (/wsf-new-milestone)
