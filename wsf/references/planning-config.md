...
| `git.phase_branch_template` | `"wsf/phase-{phase}-{slug}"` | Branch template for phase strategy |
| `git.milestone_branch_template` | `"wsf/{milestone}-{slug}"` | Branch template for milestone strategy |
...
| `manager.flags.discuss` | `""` | Flags passed to `/wsf-discuss-phase` when dispatched from manager (e.g. `"--auto --analyze"`) |
...
**Using wsf-tools.cjs (preferred):**

```bash
# Commit with automatic commit_docs + gitignore checks:
node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" commit "docs: update state" --files .planning/STATE.md

# Load config via state load (returns JSON):
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" state load)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
# commit_docs is available in the JSON output

# Or use init commands which include commit_docs:
INIT=$(node "/Users/sam/coding/wopal/wopal-workspace/.wopal/wsf/bin/wsf-tools.cjs" init execute-phase "1")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
...
**Note:** Most WSF operations use direct file reads or explicit paths, which work regardless of gitignore status.
...
- Standard WSF behavior

**When `git.branching_strategy: "phase"`:**
- `execute-phase` creates/switches to a branch before execution
- Branch name from `phase_branch_template` (e.g., `wsf/phase-03-authentication`)
...
- Branch name from `milestone_branch_template` (e.g., `wsf/v1.0-mvp`)
...
| `git.phase_branch_template` | string | `"wsf/phase-{phase}-{slug}"` | Template with `{phase}`, `{slug}` | Branch naming template for `phase` strategy |
| `git.milestone_branch_template` | string | `"wsf/{milestone}-{slug}"` | Template with `{milestone}`, `{slug}` | Branch naming template for `milestone` strategy |
...
| `features.global_learnings` | boolean | `false` | `true`, `false` | Enable injection of global learnings from `~/.wsf/learnings/` into agent prompts |
...
| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `manager.flags.discuss` | string | `""` | Any CLI flags string | Flags passed to `/wsf-discuss-phase` from manager (e.g., `"--auto --analyze"`) |
...
5. **Search API keys and flags** -- `brave_search`, `firecrawl`, and `exa_search` are auto-set to `true` during project creation if the corresponding API key is detected (environment variable or `~/.wsf/<name>_api_key` file). Setting them to `true` without the API key has no effect.
...
8. **`sub_repos` auto-sync** -- On every config load, WSF scans for child directories with `.git` and updates the `sub_repos` array if the filesystem has changed. Legacy `multiRepo: true` is automatically migrated to a detected `sub_repos` array.
