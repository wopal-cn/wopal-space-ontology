...
The `--ws <name>` flag scopes WSF operations to a specific workstream, enabling
parallel milestone work by multiple Claude Code instances on the same codebase.

## Resolution Priority

1. `--ws <name>` flag (explicit, highest priority)
2. `WSF_WORKSTREAM` environment variable (per-instance)
...
WSF now prefers a session-scoped pointer keyed by runtime/session identity
(`WSF_SESSION_KEY`, `CODEX_THREAD_ID`, `CLAUDE_CODE_SSE_PORT`, terminal session IDs,
...
When WSF resolves the session-scoped pointer in step 3 above, it uses this order:

1. Explicit runtime/session env vars such as `WSF_SESSION_KEY`, `CODEX_THREAD_ID`,
...
If none of those produce a stable identity, WSF does not keep probing. It falls
...
non-interactive, WSF intentionally skips shelling out to `tty` because that path
...
- If that was the last pointer for the repo, WSF also removes the now-empty
  per-project temp directory
- If sibling session pointers still exist, the temp directory is left in place
- When a pointer refers to a workstream directory that no longer exists, WSF
...
WSF does not currently run a background garbage collector for historical temp
...
All workflow routing commands include `${WSF_WS}` which:
...
```bash
# All wsf-tools commands accept --ws
node wsf-tools.cjs state json --ws feature-a
node wsf-tools.cjs find-phase 3 --ws feature-b

# Session-local switching without --ws on every command
WSF_SESSION_KEY=my-terminal-a node wsf-tools.cjs workstream set feature-a
WSF_SESSION_KEY=my-terminal-a node wsf-tools.cjs state json
WSF_SESSION_KEY=my-terminal-b node wsf-tools.cjs workstream set feature-b
WSF_SESSION_KEY=my-terminal-b node wsf-tools.cjs state json

# Workstream CRUD
node wsf-tools.cjs workstream create <name>
node wsf-tools.cjs workstream list
node wsf-tools.cjs workstream status <name>
node wsf-tools.cjs workstream complete <name>
```
