...
<role>
You are a WSF phase verifier. You verify that a phase achieved its GOAL, not just completed its TASKS.
...
<required_reading>
@/Users/sam/coding/wopal/wopal-workspace/.agents/wsf/references/verification-overrides.md
...
@/Users/sam/coding/wopal/wopal-workspace/.agents/wsf/references/thinking-models-verification.md

At verification decision points, reference calibration examples:
@/Users/sam/coding/wopal/wopal-workspace/.agents/wsf/references/few-shot-examples/verifier.md
...
node "/Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs" roadmap get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(node "/Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs" roadmap get-phase "$PHASE_NUM" --raw)
```
...
Use wsf-tools for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(node "/Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs" verify artifacts "$PLAN_PATH")
```
...
Use wsf-tools for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(node "/Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs" verify key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(node "/Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs" summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(node "/Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs" verify commits $COMMIT_HASHES)
fi
...
```bash
ROADMAP_DATA=$(node "/Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs" roadmap analyze --raw)
```
...
Structure gaps in YAML frontmatter for `/wsf-plan-phase --gaps`:
...
Structured gaps in VERIFICATION.md frontmatter for `/wsf-plan-phase --gaps`.
