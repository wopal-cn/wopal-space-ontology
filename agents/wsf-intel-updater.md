...
- **Cross-platform.** Use Glob, Read, and Grep tools -- not Bash `ls`, `find`, or `cat`. Bash file commands fail on Windows. Only use Bash for `node /Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs intel` CLI calls.
...
### From `/wsf-intel` Command

- **Spawned by:** `/wsf-intel` command
...
The /wsf-intel command has already confirmed that intel.enabled is true before spawning this agent. Proceed directly to Step 1.
</upstream_input>
...
- `commands/wsf/*.md` -- Command files
- `wsf/bin/` -- CLI tooling
- `wsf/workflows/` -- Workflow files
- `wsf/references/` -- Reference docs
- `hooks/*.js` -- Git hooks
...
**exports constraint:** Array of ACTUAL exported symbol names extracted from `module.exports` or `export` statements. MUST be real identifiers (e.g., `"configLoad"`, `"stateUpdate"`), NOT descriptions (e.g., `"config operations"`). If an export string contains a space, it is wrong -- extract the actual symbol name instead. Use `node /Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs intel extract-exports <file>` to get accurate exports.
...
```bash
node /Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs intel patch-meta .planning/intel/stack.json --cwd <project_root>
```
...
```bash
node /Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs intel patch-meta .planning/intel/files.json --cwd <project_root>
```
...
```bash
node /Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs intel patch-meta .planning/intel/apis.json --cwd <project_root>
```
...
```bash
node /Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs intel patch-meta .planning/intel/deps.json --cwd <project_root>
```
...
Run: `node /Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs intel validate --cwd <project_root>`
...
Run: `node /Users/sam/coding/wopal/wopal-workspace/.agents/wsf/bin/wsf-tools.cjs intel snapshot --cwd <project_root>`
